...
int main(int argc, char *argv[]){
  ...
  TCPRequestChannel* control_channel = new TCPRequestChannel("server", port);
  socklen_t sin_size;
  ...
  while(1) {  // main accept() loop
    ...
    TCPRequestChannel* client = new TCPRequestChannel(client_socket);
    thread t (handle_process_loop, client);
    t.detach (); 
  }
