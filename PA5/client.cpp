...
void patient_file_thread_function(BoundedBuffer* buffer, int m, TCPRequestChannel* chan,  string filename) {
  FileRequest fm (0,0);
...
void worker_thread_function(TCPRequestChannel* chan, BoundedBuffer* buffer, BoundedBuffer* responseBuffer, HistogramCollection* hc, int m){
...
int main(int argc, char *argv[]){
  ...
  while ((opt = getopt(argc, argv, "f:w:p:b:n:m:h:r:")) != -1) {
    switch (opt) {
      case 'f':
        filename = optarg;
        break;
  ...
  if(filename.size() > 0) {
    ...
    */
    ...
    for(int i = 0; i < w; i++) {
      TCPRequestChannel* worker = new TCPRequestChannel(host, port);
